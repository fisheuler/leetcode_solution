package solution;//Design a HashSet without using any built-in hash table libraries.
//
...
class MyHashSet {
    ...
    public void add(int key) {
        ...
        Node list = findValueByIndex(key,index);
        if(list ==null){
            Node node = new Node();
            node.key = key;
    ...
    public void addNodeToHead(Node list,Node node){
        if(list!=null){
            node.next = list;
            list.prev = node;
        }else{
    ...
    public void remove(int key) {
        ...
        Node node = findValueByIndex(key,getIndex(key,length));
        if(node!=null){

            size--;
            if(node == array[getIndex(key,length)]){
                array[getIndex(key,length)]=node.next;
            }
            if(node.prev!=null){
