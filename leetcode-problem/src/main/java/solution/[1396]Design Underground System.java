package solution;//An underground railway system is keeping track of customer travel times betwee
...
class UndergroundSystem {
    ...
    public void checkOut(int id, String stationName, int t) {
        ...
        String stationKey = stationKey(c.checkinName,c.checkoutName);

        List<Customer> customerList = startAndStopStationMap.get(stationKey);
        if(customerList!=null){
            customerList.add(c);
        }else{
            List<Customer> cL = new ArrayList<>();
            cL.add(c);
            startAndStopStationMap.put(stationKey,cL);
        }
    }
