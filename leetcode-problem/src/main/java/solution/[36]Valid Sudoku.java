package solution;//Determine if a 9 x 9 Sudoku board is valid. Only the filled cells need to be v
        ...
        for(int i=0;i<board.length;i++){
            for(int j = 0;j<board[0].length;j++){
                if(board[i][j]!='.'){
                    ...
                    if(!haveSeen.add(encodeValue + i) ||
                            haveSeen.add(j+encodeValue) ||
                    haveSeen.add(i/3 + encodeValue + j/3)){
                        return false;
                    }
                }
