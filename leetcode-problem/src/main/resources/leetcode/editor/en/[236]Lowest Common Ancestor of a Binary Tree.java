//Given a binary tree, find the lowest common ancestor (LCA) of two given nodes 
//in the tree. 
//
// According to the definition of LCA on Wikipedia: “The lowest common ancestor 
//is defined between two nodes p and q as the lowest node in T that has both p and
// q as descendants (where we allow a node to be a descendant of itself).” 
//
// 
// Example 1: 
//
// 
//Input: root = [3,5,1,6,2,0,8,null,null,7,4], p = 5, q = 1
//Output: 3
//Explanation: The LCA of nodes 5 and 1 is 3.
// 
//
// Example 2: 
//
// 
//Input: root = [3,5,1,6,2,0,8,null,null,7,4], p = 5, q = 4
//Output: 5
//Explanation: The LCA of nodes 5 and 4 is 5, since a node can be a descendant o
//f itself according to the LCA definition.
// 
//
// Example 3: 
//
// 
//Input: root = [1,2], p = 1, q = 2
//Output: 1
// 
//
// 
// Constraints: 
//
// 
// The number of nodes in the tree is in the range [2, 105]. 
// -109 <= Node.val <= 109 
// All Node.val are unique. 
// p != q 
// p and q will exist in the tree. 
// 
// Related Topics Tree Depth-First Search Binary Tree 
// 👍 6723 👎 220


//leetcode submit region begin(Prohibit modification and deletion)
/**
 * Definition for a binary tree node.
 * public class TreeNode {
 *     int val;
 *     TreeNode left;
 *     TreeNode right;
 *     TreeNode(int x) { val = x; }
 * }
 */
class Solution {
    TreeNode LCA =null;
    public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
        int value = contains(root,p,q);
        return LCA;
    }

    public int contains(TreeNode root,TreeNode p, TreeNode q){
        if(root ==null){
            return 0;
        }
        int leftContains = contains(root,p,q);

        int rightContains = contains(root,p,q);

        int rootContains = 0 ;
        if(root.val == p.val || root.val == q.val){
            rootContains=1;
        }

        int sum = leftContains + rightContains + rootContains;

        if(sum =2){
            if(LCA ==null){
                LCA = root;
            }
        }
        return sum;
    }

    public int dfs(TreeNode root,TreeNode p,TreeNode q){

        if(root == null){
            return ;
        }

        if(LCA!=null){
            return;
        }

        dfs(root.left,p,q);

        dfs(root.right,p,q);


        if(dfs(root.left,p,q)){
            LCA = root.left;
            return;
        }
        if(contains(root.right,p,q)){
            LCA = root.right;

            return;
        }

        if(contains(root,p,q)){
            LCA = root.right;
            return;

        }

    }

    public boolean contains(TreeNode root,TreeNode p,TreeNode q){
        if(contains(root,p) && contains(root,q)){
            return true;
        }
        return false;
    }

    public boolean contains(TreeNode root,TreeNode p){
        if(root==null){
            return false;
        }
        if(root.val == p.val){
            return true;
        }
        if(contains(root.left,p)){
            return true;
        }
        if(contains(root.right,p)){
            return true;
        }
        return false;
    }

}
//leetcode submit region end(Prohibit modification and deletion)


