...
class Solution {
    ...
    Map<Integer,List<Integer>> copyNodeMap = new HashMap<>();
    ...
    public Node cloneGraph(Node node) {
        ...
        for(Integer val: copyNodeMap.keySet()){
            Node node = new Node(val);
            copyNodeMap.put(val,node);
        }
        for(Map.Entry<Integer,List<Integer>> entry: copyNodeMap.entrySet()){
        ...
        return copyNodeMap.values().iterator().next();
    }

    public void dfs(Node node,Map<Integer,Boolean> visitedMap){
        if(!visitedMap.contains(node.val)){
            ...
            copyNodeMap.put(node.val,neighborList);
        }
    }
